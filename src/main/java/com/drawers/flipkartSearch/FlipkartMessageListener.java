 ...
 */
public class FlipkartMessageListener implements NewMessageListener, GroupMessageListener {
    private final DrawersBot bot;
    private final String clientId;
    private ExecutorService executorService = Executors.newFixedThreadPool(10);

    public FlipkartMessageListener(DrawersBot bot, String clientId) {
        this.bot = bot;
    ...
    public void receiveMessage(MqttChatMessage mqttChatMessage) {
        executorService.submit((Runnable) () -> {
            ...
            try {
                responseString = URLEncoder.encode(response.toString(), "UTF-8");
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
            ...
            MqttChat mqttChat = new MqttChat(mqttChatMessage.senderUid, UUID.randomUUID().toString(), responseString, false, ChatConstant.ChatType.QAR, clientId);
    ...
    public void receiveMessage(GroupMessage.GroupMessageContainer groupMessageContainer, String s) {
        executorService.submit((Runnable) () -> {
            ...
            GroupMessage groupMessage = new GroupMessage(s, UUID.randomUUID().toString(), response.toUserString(), ChatConstant.ChatType.QAR, clientId);
